...
class OidcTokenProvider {
    ...
    async getToken(options) {
        ...
        if (type === "oidc") {
            ...
            oidcToken = new CustomOidcToken(path.join(homedir, ".uucloud-cli", "work", tokenAlias),oidcOptions);
        } else {
        ...
        if (mode === "interactive") {
            ...
            while (!login) {
                let ac1 = await read({
                    prompt: `Access code 1 for ${options.user} : `,
                    silent: true,
                    output: process.stderr
                });
                let ac2 = await read({
                    prompt: `Access code 2 for ${options.user} : `,
                    silent: true,
                    output: process.stderr
                });
